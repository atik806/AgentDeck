"""Qt bridge for the GitLab plugin -- the surface ``plugins_panel`` talks to.

Mirrors ``vercel_controller.VercelController``: GitLab's hosted MCP server is
OAuth-only, and the agent owns the OAuth credentials (the user runs ``/mcp`` in a
pane). So there is no device flow, no token vault, no capability model here.
"Connecting" means: record the connection in ``plugins.json``, drop a tokenless
server entry into ``~/.claude.json``, and best-effort mirror the *metadata* to the
account's ``plugin_connections`` row.

``gitlab_mcp`` / ``plugin_store`` do the real work and stay Qt-free.
"""
...
import gitlab_mcp
from plugin_store import GITLAB, PluginConnection, PluginStore

__all__ = ["GitLabController"]
...
class GitLabController(QObject):
    """The GitLab-plugin surface. Safe to construct unconditionally."""
    ...
    def is_connected(self) -> bool:
        return self._store.is_connected(GITLAB)

    @property
    def connection(self) -> Optional[PluginConnection]:
        return self._store.get(GITLAB)
    ...
    def _target_agent_keys(self, agent_command: Optional[str] = None) -> list[str]:
        """Which agents to write the GitLab MCP server into -- the workspace's
        agent plus, unless ``plugins_wire_all_agents`` is off, every installed
        agent. ``gitlab_mcp.inject`` further filters to the OAuth-capable set."""
        import agents
    ...
    def ensure_wired(self, folder: Optional[str] = None,
        ...
        """Add the GitLab MCP server to every OAuth-capable target agent's
        ...
        if not keys:
            return False
        try:
            return gitlab_mcp.inject(agent_keys=keys)
