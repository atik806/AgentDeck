"""Qt bridge for the Linear plugin -- the surface ``plugins_panel`` talks to.

Mirrors ``vercel_controller.VercelController``: Linear's hosted MCP server is
...
``linear_mcp`` / ``plugin_store`` do the real work and stay Qt-free.
"""
...
import linear_mcp
from plugin_store import LINEAR, PluginConnection, PluginStore
...
class LinearController(QObject):
    """The Linear-plugin surface. Safe to construct unconditionally."""
    ...
    def is_connected(self) -> bool:
        return self._store.is_connected(LINEAR)

    @property
    def connection(self) -> Optional[PluginConnection]:
        return self._store.get(LINEAR)
    ...
    def _target_agent_keys(self, agent_command: Optional[str] = None) -> list[str]:
        """Which agents to write the Linear MCP server into -- the workspace's
        agent plus, unless ``plugins_wire_all_agents`` is off, every installed
        agent. ``linear_mcp.inject`` further filters to the OAuth-capable set."""
        import agents
    ...
    def ensure_wired(self, folder: Optional[str] = None,
        ...
        """Add the Linear MCP server to every OAuth-capable target agent's
        ...
        if not keys:
            return False
        try:
            return linear_mcp.inject(agent_keys=keys)
