...
check("opencode supported (phase 3)", gitlab_mcp.supports_agent("opencode"))
check("codex not yet supported (phased OAuth rollout)", not gitlab_mcp.supports_agent("codex"))
...
with tempfile.TemporaryDirectory() as d:
    cc = Path(d) / ".claude.json"
    check("codex agent -> no-op", not gitlab_mcp.inject(agent_command="codex", claude_config=cc))
