"""Wire a "connected" GitLab plugin into the coding agent(s) running in a pane.

Like ``vercel_mcp`` this writes an **MCP server** into each target agent's *user*
scope (Claude Code's root ``mcpServers`` in ``~/.claude.json``, etc.) so a pane gets
GitLab's merge-request / pipeline / issue tools natively, whatever folder it runs in.

**Thin, on purpose.** GitLab's official MCP server is *hosted* and *OAuth-only*
(``https://gitlab.com/api/v4/mcp`` -- transport ``http``, OAuth 2.0 with Dynamic
Client Registration). It does not take an API bearer token in this path, and there
is no official local binary to ship. Approved MCP clients perform the OAuth
themselves: the user authorises once in the pane (``/mcp`` for Claude, opencode
opens the browser on first use). So AgentDeck never handles a GitLab token --
"connecting" the plugin just drops a tokenless server entry into each agent's
config; authorising happens in the pane.

**gitlab.com only in v1.** A self-hosted GitLab instance points at
``https://<your-gitlab>/api/v4/mcp``; that would need a per-connection URL field --
future work. Override :data:`REMOTE_MCP_URL` meanwhile. (Mirrors Jira's
...
from plugin_store import GITLAB as _PROVIDER
...
#: GitLab's hosted MCP endpoint for gitlab.com. Self-hosted instances use
#: ``https://<host>/api/v4/mcp`` -- switch this constant for those until a
#: per-connection URL field exists.
REMOTE_MCP_URL = "https://gitlab.com/api/v4/mcp"
...
def supports_agent(agent: Optional[str]) -> bool:
    ...
    GitLab plugin can wire it). Accepts a key or a command string."""
...
def mcp_server_config() -> dict:
    """The concrete ``mcpServers["gitlab"]`` block for **Claude Code**."""
...
def inject(
    *,
    ...
    """Add the GitLab MCP server to each OAuth-capable target agent's config.

    * No-op (returns False) when no target could be written.
    * Idempotent per agent.
    * Refuses to touch a ``gitlab`` server the user configured themselves.
    """
...
def remove(
    *,
    ...
    """Drop the AgentDeck-managed GitLab server from every agent config we wrote it
