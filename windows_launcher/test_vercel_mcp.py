...
check("codex supported (all MCP agents wired)", vercel_mcp.supports_agent("codex"))
check("gemini supported", vercel_mcp.supports_agent("gemini"))
...
with tempfile.TemporaryDirectory() as d:
    cc = Path(d) / ".claude.json"
    check("aider agent -> no-op (no MCP support)", not vercel_mcp.inject(agent_command="aider", claude_config=cc))
