"""Qt bridge for the Jira plugin -- the surface ``plugins_panel`` talks to.

Mirrors ``vercel_controller.VercelController``: Atlassian's Rovo MCP server is
hosted and OAuth-only, and Claude Code owns the OAuth credentials (the user runs
``/mcp`` in a pane). So there is no device flow, no token vault, no capability
model here. "Connecting" means: record the connection in ``plugins.json``, drop a
tokenless server entry into ``~/.claude.json``, and best-effort mirror the
*metadata* to the account's ``plugin_connections`` row.

``jira_mcp`` / ``plugin_store`` do the real work and stay Qt-free.
"""
...
import jira_mcp
from plugin_store import JIRA, PluginConnection, PluginStore

__all__ = ["JiraController"]
...
class JiraController(QObject):
    """The Jira-plugin surface. Safe to construct unconditionally."""
    ...
    def is_connected(self) -> bool:
        return self._store.is_connected(JIRA)

    @property
    def connection(self) -> Optional[PluginConnection]:
        return self._store.get(JIRA)
