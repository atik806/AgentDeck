"""Real pseudo-console sessions -- one per pane. Windows implementation.
...
Selected by ``pty_backend.py`` on ``sys.platform == "win32"`` -- see that module
for the platform dispatch and the full public contract (``_pty_backend_posix.py``
is the Linux/macOS sibling implementing the same contract).
"""
