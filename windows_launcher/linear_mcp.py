"""Wire a "connected" Linear plugin into the coding agent(s) running in a pane.
...
Linear's issue / project / comment tools natively, whatever folder it runs in.

**Thin, on purpose.** Linear's official MCP server is *hosted* and *OAuth-only*
(``https://mcp.linear.app/mcp`` -- streamable ``http``, OAuth 2.1 with Dynamic
Client Registration). It does not take an API bearer token, and there is no
official local binary. Approved MCP clients perform the OAuth themselves: the user
authorises once in the pane (``/mcp`` for Claude, opencode opens the browser on
first use). So AgentDeck never handles a Linear token -- "connecting" the plugin
just drops a tokenless server entry into each agent's config; authorising happens
in the pane.

The endpoint is read-write. Linear also serves a read-only surface at
``https://mcp.linear.app/mcp/readonly`` -- a future per-connection toggle; switch
:data:`REMOTE_MCP_URL` meanwhile.
...
from plugin_store import LINEAR as _PROVIDER
...
#: Linear's hosted MCP endpoint (read-write). ``.../mcp/readonly`` exposes only
#: read tools -- switch this constant for a read-only connection.
...
def supports_agent(agent: Optional[str]) -> bool:
    ...
    Linear plugin can wire it). Accepts a key or a command string."""
...
def mcp_server_config() -> dict:
    """The concrete ``mcpServers["linear"]`` block for **Claude Code**."""
...
def inject(
    *,
    ...
    """Add the Linear MCP server to each OAuth-capable target agent's config.

    * No-op (returns False) when no target could be written.
    * Idempotent per agent.
    * Refuses to touch a ``linear`` server the user configured themselves.
    """
...
def remove(
    *,
    ...
    """Drop the AgentDeck-managed Linear server from every agent config we wrote it
