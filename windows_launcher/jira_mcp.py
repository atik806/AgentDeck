"""Wire a "connected" Jira plugin into the coding agent(s) running in a pane.

Like ``vercel_mcp`` this writes an **MCP server** into each target agent's *user*
scope so a pane gets Jira / Confluence tools natively, whatever folder it runs in.
...
covers Jira, Confluence, Jira Service Management, Bitbucket and Compass. It does not
take a plain bearer token in v1, and there is no official local binary. Approved MCP
clients perform the OAuth themselves: the user authorises once in the pane. So
AgentDeck never handles an Atlassian token -- "connecting" the plugin just drops a
...
from plugin_store import JIRA as _PROVIDER
...
def supports_agent(agent: Optional[str]) -> bool:
    ...
    Jira plugin can wire it). Accepts a key or a command string."""
...
def mcp_server_config() -> dict:
    """The concrete ``mcpServers["atlassian"]`` block for **Claude Code**."""
...
def inject(
    *,
    ...
    """Add the Atlassian MCP server to each OAuth-capable target agent's config.

    * No-op (returns False) when no target could be written.
    * Idempotent per agent.
    * Refuses to touch an ``atlassian`` server the user configured themselves.
    """
...
def remove(
    *,
    ...
    """Drop the AgentDeck-managed Atlassian server from every agent config we wrote
