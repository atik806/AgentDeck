...
class AudioCapture:
    ...
    def start(self) -> None:
        ...
        self._transcribe_thread = threading.Thread(
            target=self._transcribe_loop, name="voice-transcribe", daemon=True
        )
    ...
    def stop(self) -> None:
        """Stop capture, flush any in-progress utterance, join workers."""
        ...
        # Unblock the capture loop and let it drain.
        self._audio_queue.put(None)
        if self._capture_thread is not None:
            self._capture_thread.join(timeout=2.0)
            self._capture_thread = None

        # Flush a partially-spoken utterance, then stop the transcriber.
        self._flush_segment()
